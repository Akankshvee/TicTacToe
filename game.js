const Board = () => {
    ...
    let winVisibility = checkForWinner(gameState);
    let setWinVisibility = winVisibility != 'No Winner Yet' ? "visible" :  "hidden";
    let status = `Winner is ${winVisibility}`;
    console.log('gamestate', setWinVisibility);
    let nxtPlayer = 'X';

    // Part 1 step 1 code goes here
    // Use conditional logic to set a variable to either 'Player O' or  'Player X'
    if(player == 0) nxtPlayer = 'O';
    else nxtPlayer = 'X';
    ...
    const takeTurn = (id) => {
      ...
      console.log('return player', player);
      return player;    
    };

    function renderSquare(i) {
      // use properties to pass callback function takeTurn to Child
      return <Square takeTurn={takeTurn} id={i} win={setWinVisibility}></Square>;
    }
    ...
    return (
      <div>
          ...
          <div id="info">
         <h1 id="turn" className="white">Next Player: Player {nxtPlayer}</h1>
         <h1 className="winner" id="winner" style={{
            visibility: setWinVisibility
            }}>{status}</h1>
        </div>
      ...
      <button onClick={reset} className="restart">Restart Game</button>
      </div>
    );
  };
  
  const Square = ({ takeTurn, id, win }) => {
    const mark = ['O', 'X', '+'];
    ...
    return (
      <button
        ...
        onClick={() => {
            if(!filled & win == "hidden") {
          setTik(takeTurn(id));
          setFilled(true);
          console.log(`Square: ${id} filled by player : ${tik} ${filled}`);
            }
